...
socket_t hostname_connect(const std::string& hostname, int port) {
    ...
    if ((ret = getaddrinfo(hostname.c_str(), sport, &hints, &result)) != 0)
    {
      ...
      LOG(ERROR) << "ERROR getaddrinfo " << gai_strerror(ret);
      return 1;
    }
  ...
  public:
    ...
    void poll(int timeout) { // timeout in milliseconds
        ...
        while (true) {
            ...
            int N = rxbuf.size();
            ssize_t ret;
            rxbuf.resize(N + chunk_size);

            ret = recv(sockfd, (char*)&rxbuf[0] + N, chunk_size, 0);
        ...
        while (true) {
            ...
            if (ws.N0 < 126) {
                ...
                if (ws.N & 0x8000000000000000ull) {
                    ...
                    // for now.
                    isRxBad = true;
                    LOG(ERROR) << "Frame has invalid frame length";
            ...
            if (false) { }
                ...
                LOG(DEBUG) << "wsheader_type::PING-PONG";
            }
            else if (ws.opcode == wsheader_type::PONG) { LOG(DEBUG) << "wsheader_type::PONG"; }
            else if (ws.opcode == wsheader_type::CLOSE) { LOG(ERROR) << "wsheader_type::CLOSE"; close(); }
            else { fprintf(stderr, "ERROR: Got unexpected WebSocket message.\n"); LOG(ERROR) << "Got unexpected WebSocket message"; close(); }
...
easywsclient::WebSocket::pointer from_url(const std::string& url, bool useMask, const std::string& origin) {
    ...
    if (url.size() >= 512) {
      ...
      LOG(ERROR) << "ERROR: url size limit exceeded: " << url.c_str();
      return NULL;
    }
    if (origin.size() >= 200) {
      fprintf(stderr, "ERROR: origin size limit exceeded: %s\n", origin.c_str());
      LOG(ERROR) << "ERROR: origin size limit exceeded:" << origin.c_str();
      return NULL;
    }
    if (false) { }
        ...
        LOG(ERROR) << "ERROR: Could not parse WebSocket url: " << url.c_str();
        return NULL;
    }
